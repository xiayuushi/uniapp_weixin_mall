	...
	export default {
		...
		methods: {
			...
			async createPayOrder () {
				const goods = this.checkCartSelected.map(item => { return { goods_id: item.goods_id, goods_price: item.goods_price, goods_number: item.num} })
				const create_order_res = await this.$request({
					url: '/my/orders/create',
					method: 'post',
					addToken: true,
					data: { order_price: this.totalPrice, consignee_addr: this.address, goods },
				})
				
				this.DELPAYCART() // 移除购物车列表中参与结算的商品
				
				const { order_number } = create_order_res
				const { message } = await this.$request({ url: '/my/orders/req_unifiedorder', method: 'post', addToken: true, data: { order_number } }) 
